...
| 20 | FACe (SSPP) | 🇪🇸 Spain | 🔴 | ▫️ | FNMT cert = human ID check; WS-Security signing IS implemented (live-proved 2026-09-02) — the cert must be FACe-registered |
...
**How this maps onto the app's own "anaf" channel (Settings → Channels)** — the four secrets above are what a `.live.spec.ts` reads from the CI environment; the deployed app's own connected channel (`CompanyChannelConfig`, `transports/anaf-transport.ts#extractAnafCredentials`) stores four *slightly different* fields for the exact reason the "Hard blocker for CI/automation" bullet below names: nothing can keep re-running the interactive certificate flow to mint a fresh 90-day `ANAF_AUTH_TOKEN` forever. So the app stores `cif` (this section's own CUI/CIF), `clientId`/`clientSecret` (this section's own OAuth pair), and a `refreshToken` (the 365-day refresh token step 8 below also produces) — never a pasted access token — and silently exchanges that refresh token for a fresh access token itself (`POST .../token`, `grant_type=refresh_token`, step 11 below) on every send. A future `anaf.live.spec.ts` exercising the APP's own transport should therefore read `ANAF_REFRESH_TOKEN` (not yet a secret this repo defines — add it alongside the four above, following the refresh-token capture in step 8) rather than trying to reuse a short-lived `ANAF_AUTH_TOKEN` directly.
...
> **GitHub secrets:** `FACE_CERTIFICATE`, `FACE_CERTIFICATE_PASSWORD`, `FACE_NOTIFICATION_EMAIL` &nbsp;•&nbsp; **Live flag:** `FACE_LIVE=1` &nbsp;•&nbsp; **Sandbox:** yes, and REACHABLE without any credential (see below) &nbsp;•&nbsp; **Repo status:** 🔴 missing (the WS-Security TRANSPORT signature is now implemented and live-verified as of 2026-09-02 — see the warning below for what STILL blocks a genuine accepted deposit)
...
**⚠️ Even with all three secrets set, this codebase's own `enviarFactura` call is STILL expected to fail — but for a NARROWER, different reason than before 2026-09-02.** FACe's SSPP web service authenticates every request with a **WS-Security X.509 signature** over the SOAP envelope (`<wsse:BinarySecurityToken>` + `<ds:Signature>` covering the `<soapenv:Body>`, OASIS X.509 Certificate Token Profile — see `transports/face/wsse-sign.ts`'s own header for the cited form). As of a 2026-09-02 task, this codebase GENUINELY BUILDS that signature (`transports/face-transport.ts#FaceSoapHttpPort`, `wsse-sign.ts`), using the SAME company signing certificate already resolved for the Facturae document's own XAdES signature (`modules/company/signing-certificates/`, applicability "XAdES"/"*" — see `face-transport.ts`'s own header, "THE WS-SECURITY CERTIFICATE", for why that reuse is deliberate and what it does NOT unify: the FACe channel's own `FACE_CERTIFICATE` above remains a SEPARATE credential, still used only for mTLS). What remains genuinely missing is a **FACe-registered certificate**: this task LIVE-PROVED (no credential needed — a throwaway self-signed test certificate) that a WS-Security-signed request makes the sandbox's fault CHANGE from `"not signed"` to `"error validating the certificate"` — i.e. the signing layer now WORKS, but any certificate not actually issued-and-registered through the FNMT/FACe process below will always fail at THAT point. `face/face.live.spec.ts`'s own gated `describeReachability` block reproduces this exact proof on every `FACE_LIVE=1` run, credential-free.
...
5. Base64-encode the `.pfx` → `FACE_CERTIFICATE`; the password chosen at export → `FACE_CERTIFICATE_PASSWORD`. Upload the SAME `.pfx` at Company settings → Signing certificates (applicability "XAdES" or "*") too — see the warning above for why the WS-Security signature reuses THAT credential slot, not this one.
...
2. Once registered, the SAME certificate authenticates every SSPP call at the WS-Security layer (see the warning above — this codebase now DOES compute that signature; registration with FACe is what makes the CERTIFICATE ITSELF something FACe's own trust store recognizes).

**What IS independently, live-verified without any credential at all** — see `face/face.live.spec.ts`'s own header for the full account:
- (2026-09-02) The sandbox host `https://se-face-webservice.redsara.es/facturasspp2` is reachable and answers an UNSIGNED SOAP request with a real Fault, `faultcode 401`, `faultstring "La petición no esta firmada"`. The HTTP status code itself was observed to flip between `200` and `500` across repeated calls for the identical fault body (a load-balanced backend, evidently) — `face-client.ts`'s own response parsing accounts for this (tries to parse a Fault regardless of status).
- (2026-09-02, SAME task, second half) A WS-Security-**SIGNED** SOAP request — `wsse-sign.ts`'s real OASIS-form signature, a throwaway in-memory self-signed test certificate, NO FACe registration at all — gets a DIFFERENT Fault from the SAME sandbox: `faultcode 401`, `faultstring "Error al validar el certificado"`. The faultCODE is unchanged (401 both times) but the faultSTRING is a genuinely different sentence — proof the WS-Security layer is now PARSED far enough to reach certificate validation, which only a certificate FACe actually recognizes (i.e. FNMT-issued AND FACe-registered) can pass. Incidentally, the sandbox's OWN Fault response is itself WS-Security-signed with a real FNMT "SELLO ENTIDAD SGAD PRUEBAS" test certificate, confirming the OASIS X.509 Token Profile form cited in `wsse-sign.ts` is genuinely what FACe's own stack speaks (that response also signs a `wsu:Timestamp` alongside the Body, using RSA-SHA1/SHA1 rather than SHA-256 — our SHA-256, Timestamp-less request was still accepted far enough to reach certificate validation, so FACe's own INCOMING checks are evidently more lenient than its own outgoing convention).
- **Also observed** (2026-09-02): the HUMAN-FACING `face.gob.es` portal now redirects to a successor, `proveedores.face.gob.es`, with a banner "en este portal solo se pueden consultar facturas remitidas hasta el 27/02/2026" — a **portal migration for invoice CONSULTATION**; nothing found today indicates the machine SSPP SOAP endpoints above have themselves moved, but this was not independently confirmed against a production credential either — worth re-checking before relying on the endpoint constants in `transports/face-transport.ts#FACE_ENDPOINTS` for a real production rollout.
...
- **Remaining blocker for THIS codebase specifically, distinct from credential acquisition**: WS-Security XML-DSig signing of the SOAP envelope is IMPLEMENTED and live-proved to reach certificate validation (see above) — the remaining gap is narrower than before: an actual FNMT-issued, FACe-**registered** certificate. A company that obtains and registers one is expected to reach whatever FACe's real acceptance path looks like (a genuine deposit, or a different, more specific rejection such as DIR3 routing) — this task could not observe which, having no such certificate to test with.
