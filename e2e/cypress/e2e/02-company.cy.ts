...
describe('Company Settings E2E', () => {
    describe('1 - Initial Company Setup (Required for other tests)', () => {
        it('creates the company via onboarding', () => {
            ...
            cy.get('[data-cy="onboarding-company-quote-number-format-input"]').should('have.value', 'Q-{year}-{number}');

            // Invoice numbering
            cy.get('[data-cy="onboarding-company-invoice-starting-number-input"]').clear().type('1');
            cy.get('[data-cy="onboarding-company-invoice-number-format-input"]').should('have.value', 'INV-{year}-{number}');

            // Receipt numbering
            cy.get('[data-cy="onboarding-company-receipt-starting-number-input"]').clear().type('1');
            cy.get('[data-cy="onboarding-company-receipt-number-format-input"]').should('have.value', 'REC-{year}-{number}');
    ...
    describe('4 - Dynamic Identifiers Based on Country', () => {
        it('displays dynamic identifier fields for France', () => {
            cy.visit('/settings/company');
            cy.wait(3000);
            cy.get('[data-cy="company-name-input"]', { timeout: 15000 }).should('be.visible');

            // France has SIRET and VAT identifiers
            cy.get('[data-cy="company-siret-input"]').should('be.visible');
            cy.get('[data-cy="company-vat-input"]').should('be.visible');
        });

        it('validates identifier formats based on country config', () => {
            cy.visit('/settings/company');
            cy.wait(3000);

            // France SIRET must be 14 digits
            cy.get('[data-cy="company-siret-input"]', { timeout: 10000 }).clear().type('123');
            cy.get('[data-cy="company-submit-btn"]').click();
    ...
    describe('5 - Number Format Configuration', () => {
    ...
    describe('6 - PDF Format Selection', () => {
    ...
    describe('7 - VAT Exemption Toggle', () => {
    ...
    describe('8 - Restore Valid State (Must run last)', () => {
        it('restores valid company settings for other tests', () => {
            ...
            cy.get('[data-cy="company-quote-number-format-input"]').clear().type('Q-{year}-{number}');
            cy.get('[data-cy="company-invoice-starting-number-input"]').clear().type('1');
            cy.get('[data-cy="company-invoice-number-format-input"]').clear().type('INV-{year}-{number}');
            cy.get('[data-cy="company-receipt-starting-number-input"]').clear().type('1');
            cy.get('[data-cy="company-receipt-number-format-input"]').clear().type('REC-{year}-{number}');
            ...
            cy.get('[data-cy="company-name-input"]', { timeout: 15000 }).should('be.visible');
            cy.get('[data-cy="company-name-input"]').should('have.value', 'Acme Corp');
            cy.get('[data-cy="company-siret-input"]').should('have.value', '12345678901234');
