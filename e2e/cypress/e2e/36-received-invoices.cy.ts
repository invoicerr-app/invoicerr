 ...
 * WITHOUT any real OCR engine (`ocr/fake-extractor.ts`, see this file or its own header). Both also
 * prove the OCR runs in a background job: the upload answers "pending" with no field, fast, and the
 * form fills in once the dialog's poll of the job's status route reports it done.
 *
...
/**
 * OCR runs in a background job, never inside the upload request: under load the request used to
 * wait for Tesseract (a 10-user load test cut uploads at 60 s). Spies on the upload and on the
 * status route the dialog polls, and records how long the upload itself took, so a test can prove
 * that the upload answered with "pending" and the fields came later, through the poll.
 */
function spyOnAsyncOcr() {
	const timing = { uploadMs: -1 };
	cy.intercept("POST", "**/api/documents/received-invoices/upload", (req) => {
		const startedAt = Date.now();
		req.continue(() => {
			timing.uploadMs = Date.now() - startedAt;
		});
	}).as("upload");
	cy.intercept("GET", "**/api/documents/received-invoices/upload/*/ocr").as("ocrPoll");
	return cy.wrap(timing);
}

/** The upload must have answered "pending" with nothing extracted yet, fast — the proof that the
 *  OCR did not run inside the request. Returns the uploaded file's `fileRef`. */
function expectUploadAnsweredPending(timing: { uploadMs: number }) {
	return cy.wait("@upload").then(({ response }) => {
		expect(response?.statusCode, "upload").to.eq(201);
		expect(response?.body.ocr, "the OCR is queued, not run in the request").to.deep.equal({
			outcome: "pending",
		});
		expect(response?.body.extraction.fields, "nothing extracted by the upload itself").to.deep.equal({});
		expect(timing.uploadMs, "the upload answers without waiting for the OCR").to.be.within(0, 3000);
		return response?.body.fileRef as string;
	});
}
...
describe("Receiving invoices", () => {
	...
	it("a plain PDF (no embedded XML): empty form but the file is still attached, never a refusal", () => {
		spyOnAsyncOcr().then((timing) => {
			uploadAndOpenForm(PLAIN_PDF_FIXTURE);
			expectUploadAnsweredPending(timing);
		});
		...
		// preserved by the three empty-field assertions right below. The test stub declines this
		// file, so the job ends "unavailable" and the toast arrives once the poll sees it.
		cy.get('[data-sonner-toast]', { timeout: 15000 }).should("contain.text", "OCR");
		cy.get('[data-cy="received-invoice-ocr-pending"]').should("not.exist");
	...
	it("A PDF marked for the test OCR stub arrives pre-filled via OCR, and stays editable", () => {
		spyOnAsyncOcr().then((timing) => {
			...
			expectUploadAnsweredPending(timing).then((fileRef) => {
				// The fields below can only have come from the job, through the poll: the upload
				// answered with none. The job's own result is read back by the API, not the screen.
				cy.wait("@ocrPoll");
				cy.request({ url: `${api}/api/documents/received-invoices/upload/${fileRef}/ocr` })
					.its("body")
					.then((body) => {
						expect(body.status).to.eq("done");
						expect(body.ocr.outcome).to.eq("extracted");
						expect(body.extraction.fields.supplier).to.eq("OCR Fake Fournisseur SARL");
					});
			});
		});
