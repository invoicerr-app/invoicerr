...
describe("Settlement — a SENT credit note reduces what an invoice owes, a DRAFT one settles nothing", () => {
			...
			.then((clients: { id: string }[]) => {
				...
				cy.request({
					...
					cy.request({
						...
						// below, never guessed.
						const lines = sent.body.document.data.lines as {
							$rowId: string;
							description: string;
						}[];
						const lineA = lines.find((line) => line.description === "Ligne A");
						expect(lineA, "la ligne A existe et porte un $rowId").to.not.be
							.undefined;
						...
						cy.get('[data-cy="document-settlement-badge"]', {
							timeout: 15000,
						}).should("contain.text", "Partially paid");
						...
						cy.request({
							...
							.then((body) => {
								expect(
									...
									"brut : 100 € net + 20 % de TVA",
								).to.eq(INVOICE_GROSS_MINOR);
	...
	it("a DRAFT credit note, created via the API and referencing line A, changes NOTHING in the balance (API assertion)", () => {
		expect(invoiceId, "la facture du test précédent existe toujours").to.be.a(
			"string",
		);
		expect(lineARowId, "le $rowId de la ligne A a été relevé").to.be.a(
			"string",
		);
		...
		cy.request({
			...
			expect(creditNoteId, "le brouillon d'avoir a un identifiant").to.be.a(
				"string",
			);
			expect(saved.body.document.status, "l'avoir est bien un brouillon").to.eq(
				"draft",
			);
			...
			cy.request({
				...
				.then((body) => {
					expect(body.settlement.paidMinor).to.eq(PAYMENT_MINOR);
					expect(
						body.settlement.creditedMinor,
						"un avoir en brouillon ne crédite rien",
					).to.eq(0);
					expect(body.settlement.outstandingMinor).to.eq(
						INVOICE_GROSS_MINOR - PAYMENT_MINOR,
					);
					...
					expect(
						body.credits,
						"aucun avoir compté tant qu'il est en brouillon",
					).to.have.length(0);
				});
		});
	});
	...
	it('a REAL click on "Send" for the credit note lowers the invoice\'s balance — paidMinor NEVER inflated by the credit note', () => {
		expect(creditNoteId, "l'avoir du test précédent existe toujours").to.be.a(
			"string",
		);
		...
		cy.request({
			...
			.then((body) => {
				...
				expect(
					body.settlement.paidMinor,
					"paidMinor n'a pas bougé — l'avoir n'est pas un paiement",
				).to.eq(PAYMENT_MINOR);
				expect(body.settlement.creditedMinor).to.eq(LINE_A_GROSS_MINOR);
				expect(body.settlement.paidMinor + body.settlement.creditedMinor).to.eq(
					INVOICE_GROSS_MINOR,
				);
				expect(body.settlement.outstandingMinor).to.eq(0);
				expect(
					body.settlement.excessMinor,
					"réglée exactement, aucun excédent",
				).to.eq(0);
				expect(body.settlement.settled).to.eq(true);

				expect(body.credits).to.have.length(1);
				expect(body.credits[0]).to.include({
					id: creditNoteId,
					amountMinor: LINE_A_GROSS_MINOR,
				});
	...
	it('on screen: the THREE blocks (payments, credits, balance) are displayed separately, and the badge becomes "Settled"', () => {
		expect(
			invoiceId,
			"la facture des tests précédents existe toujours",
		).to.be.a("string");
		...
		cy.get('[data-cy="document-settlement-badge"]').should(
			"contain.text",
			"Settled",
		);
		cy.get('[data-cy="document-settlement-outstanding"]').should(
			"contain.text",
			"0.00 EUR",
		);
		...
		cy.get('[data-cy="document-settlement-payments-list"]').within(() => {
			cy.get('[data-cy^="document-settlement-payment-"]').should(
				"have.length",
				1,
			);
		});
		cy.get('[data-cy="document-settlement-payments-list"]').should(
			"contain.text",
			"48.00 EUR",
		);
		...
		cy.get('[data-cy="document-settlement-credits-list"]').within(() => {
			cy.get('[data-cy^="document-settlement-credit-"]').should(
				"have.length",
				1,
			);
		});
		...
		cy.get('[data-cy="document-settlement-credited"]').should(
			"contain.text",
			"72.00 EUR",
		);
	});
		...
		cy.request({ url: `${api}/api/documents/references/client/search` })
			...
			.then((clients: { id: string; label: string }[]) => {
				...
				cy.request({
					...
					// "issueDate" (still unconditionally `required`) is the only field left on
					// "Details" now: "invoice" (OPTIONAL now that a credit note can be a FREE one, with
					// nothing to correct at all) shares the "Lines" step with "correctedLines" instead —
					// the `rowSelection` field it feeds has nothing to offer before it resolves — and
					// "currency" FOLLOWS "invoice" onto that SAME step too (both moves are
					// document-create-dialog.tsx's own `buildFieldGroups`, see that function's own
					// header for the full "why": a lock that only re-applies while both fields are
					// mounted together must never be split across two different steps).
					...
					cy.get('[data-cy="document-field-invoice-input"] button')
						.first()
						.click({ force: true });
					cy.get('[data-cy="document-field-invoice-input-options"]', {
						timeout: 10000,
					}).should("be.visible");
					cy.get('[data-cy="document-field-invoice-input"] input').type(
						client.label || client.id,
					);
					cy.get('[data-cy="document-field-invoice-input-options"] button', {
						timeout: 10000,
					})
						.first()
						.click();

					// THE PROOF that matters: the currency field now reads "USD" — the invoice's own
					// currency, never left blank or at some unrelated default — and is DISABLED, so the
					// user cannot even attempt to type a mismatch. Both fields are on the SAME step now,
					// so the lock (`SelectField`'s own `useEffect`, primitive-fields.tsx) is live here,
					// exactly like it was before "invoice" became optional.
					cy.get('[data-cy="document-field-currency-input"] button', {
						timeout: 10000,
					})
						.should("be.disabled")
					...
					// "notes"/"reason" (both optional once "invoice" IS set — `reason` is
					// `requiredIfAbsent: "invoice"`, so it stays silent here) are the "Options" step.
					...
					cy.wait("@saveCreditNoteDraft").then((interception) => {
						...
						cy.request({
							...
							.then((doc) => {
								expect(
									doc.data?.currency,
									"l'avoir est bien en USD, comme sa facture",
								).to.eq("USD");
							});
					});
				});
