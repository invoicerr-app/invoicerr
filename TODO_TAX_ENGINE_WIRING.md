...
> It deliberately does **not** touch invoice status/lifecycle/clearance/persistence — see "Out of
> scope" below.
...
- `backend/src/modules/invoices/invoices.service.ts:113,201` (`createInvoice`, `editInvoice`)
- `backend/src/modules/quotes/quotes.service.ts:135,210` (`createQuote`, `editQuote`-equivalent)
- `backend/src/modules/recurring-invoices/recurring-invoices.service.ts:51,127`
...
2. **No cross-border tax logic exists at all.** A French company invoicing a German B2B client with a
   valid VAT number should reverse-charge (0%, mention "autoliquidation"); today it just charges
   whatever flat `vatRate` the user typed on the line. Same for OSS (B2C digital/EU), exports, US
   nexus/sales tax — the engine already handles all of these
...
this is **not** a behavior change for the common case (FR domestic, standard or reduced rate); it only
corrects the cross-border and non-FR-exemption cases that are wrong today.

## Out of scope (do not touch)

- `InvoiceStatus` enum, `DRAFT` gating, restricting `editInvoice` to drafts.
- Creating `ComplianceDocument` rows, calling `ComplianceService`/the lifecycle runtime, clearance,
...
- Any frontend change. `Company.country`/`Client.country` stay free-text inputs; this pass only adds
  an **optional** `countryCode` escape hatch on the backend (nobody is forced to fill it in yet).
...
shared dev DB — see how the live-DB compliance tests are gated, same precaution applies). Additive
only, no backfill SQL.
...
- Trim + case-insensitive exact match only — **no fuzzy/Levenshtein matching**, no guessing. Return
  ...
  — so an `undefined` here is a safe, expected outcome, not an error).
...
- Add `backend/src/utils/country-name-to-iso.spec.ts`: table-driven tests for a representative sample
  + the already-a-code passthrough + the no-match-returns-undefined case.
...
1. FR company, not exempt, FR client, flat 20% → same totals as today's `calculateDiscountedTotals`
   (sanity/no-regression check on the common case).
...
4. FR company → DE B2B client, both with a VAT identifier → reverse charge: rate `0`, a
   "REVERSE_CHARGE"-style mention present, `buyerSelfAssess` reflected (check via `plan.tax` if
   exposed, otherwise via the resulting rate/mentions).
...
6. Empty/unresolvable country codes on both sides → resolves without throwing, `totalVAT` is some
   defined number (not `NaN`), and `warnings` is non-empty.
...
- Call `resolveInvoiceTax({ ... })` instead, passing:
  ...
  - `currency`, `issueDate: new Date()` (or whatever date field the existing call site already has
    available — invoices/quotes don't track a separate issue date today, `new Date()` at creation
    time is fine), `discountRate: clampDiscountRate(...)` (keep using the existing
    `clampDiscountRate` from `@/utils/financial` — don't duplicate that logic).
...
- `cd backend && npx jest src/modules/invoices src/modules/quotes src/modules/recurring-invoices src/compliance/integration src/utils/country-name-to-iso.spec.ts` — all green.
...
- Manually exercise the API (or add an e2e-ish test) for: FR company not exempt + FR client (baseline,
  unchanged); FR company exempt (unchanged, 0%); **DE company exempt** (now correctly 0% — the bug
  fix); FR company + DE B2B client with VAT numbers on both sides (now reverse-charged instead of
  flat 20%).
...
- The diff touches only: `backend/prisma/schema.prisma` (additive), the new
  ...
  (`invoices`/`quotes`/`recurring-invoices`), their DTOs, and the company/client DTOs+services+
  controllers for the optional `countryCode` field. Nothing else under `backend/src/compliance/**`
  changes.
