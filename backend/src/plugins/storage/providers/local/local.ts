...
export class LocalStorageProvider implements IStorageProvider {
    ...
    async getConfig(): Promise<ILocalConfig> {
        ...
        if (!config || !config.storagePath) {
            throw new Error('Local Storage plugin is not configured. Please provide a storage path.');
        }
    ...
    async validatePlugin(config: ILocalConfig): Promise<boolean> {
        ...
        try {
            ...
            throw new Error(`Unable to access storage path: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
