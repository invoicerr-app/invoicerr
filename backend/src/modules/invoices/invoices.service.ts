...
export class InvoicesService {
	...
	async getInvoices(page: string) {
		...
		const invoicesWithPM = await Promise.all(
			// biome-ignore lint/suspicious/noExplicitAny: Prisma type with relations is complex
			invoices.map(async (inv: any) => {
				if (inv.paymentMethodId) {
					const pm = await prisma.paymentMethod.findUnique({
						where: { id: inv.paymentMethodId },
					});
	...
	async createInvoice(body: CreateInvoiceDto) {
		...
		if (!company) {
			logger.error("No company found. Please create a company first.", {
				category: "invoice",
			});
		...
		let totalTTC = items.reduce(
			(sum, item) =>
				sum + item.quantity * item.unitPrice * (1 + (item.vatRate || 0) / 100),
			0,
		);
	...
	async editInvoice(body: EditInvoicesDto) {
		...
		if (!id) {
			logger.error("Invoice ID is required for editing", {
				category: "invoice",
			});
	...
	async getInvoicePdf(id: string): Promise<Uint8Array> {
		...
		if (invoice.paymentMethodId) {
			...
			if (
				paymentMethodName &&
				paymentMethodLabels[paymentMethodName.toUpperCase()]
			) {
				paymentMethodName =
					paymentMethodLabels[paymentMethodName.toUpperCase()];
			}
		...
		const html = template({
			...
			vatExemptText:
				invoice.company.exemptVat &&
				(invoice.company.country || "").toUpperCase() === "FRANCE"
	...
	async getInvoiceXMLFormat(id: string): Promise<EInvoice> {
		...
		if (invRec.client.type === "COMPANY") {
			...
			const personContact: business.TPerson = {
				type: "person",
				name:
					`${invRec.client.contactFirstname} ${invRec.client.contactLastname}` ||
					"N/A",
				description: invRec.client.description || "N/A",
	...
	async markInvoiceAsPaid(invoiceId: string) {
		...
		try {
			logger.info(
				`Uploading paid invoice ${invoiceId} to storage providers...`,
				{ category: "invoice" },
			);
			const pdfBuffer = await this.getInvoicePdf(invoiceId);
			const uploadedUrls = await StorageUploadService.uploadPaidInvoicePdf(
				invoiceId,
				pdfBuffer,
			);
	...
	async sendInvoiceByEmail(invoiceId: string) {
		...
		if (!invoice.client?.contactEmail) {
			logger.error("Client has no email configured; invoice not sent", {
				category: "invoice",
			});
		...
		try {
			...
			throw new BadRequestException(
				"Failed to send invoice email. Please check your SMTP configuration.",
			);
		}
