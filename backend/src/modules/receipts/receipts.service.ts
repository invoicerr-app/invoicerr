...
import {
	CreateReceiptDto,
	EditReceiptDto,
} from "@/modules/receipts/dto/receipts.dto";
...
export class ReceiptsService {
	...
	private async checkInvoiceAfterReceipt(invoiceId: string) {
		...
		if (invoice.status === "UNPAID") {
			...
			const totalPaid = receipts.reduce(
				(sum, receipt) => sum + receipt.totalPaid,
				0,
			);
	...
	async createReceiptFromInvoice(invoiceId: string) {
		...
		try {
			...
			this.logger.error(
				"Failed to dispatch RECEIPT_CREATED_FROM_INVOICE webhook",
				error,
			);
		}
	...
	async getReceiptPdf(receiptId: string): Promise<Uint8Array> {
		...
		if (receipt.invoice.client.name.length === 0) {
			receipt.invoice.client.name =
				receipt.invoice.client.contactFirstname +
				" " +
				receipt.invoice.client.contactLastname;
		}
		...
		if (receipt.paymentMethodId) {
			const pm = await prisma.paymentMethod.findUnique({
				where: { id: receipt.paymentMethodId },
			});
			if (pm) {
		...
		const html = template({
			...
			invoiceNumber:
				receipt.invoice?.rawNumber || receipt.invoice?.number?.toString() || "",
			...
			items: receipt.items.map((item) => {
				const invoiceItem = receipt.invoice.items.find(
					(i) => i.id === item.invoiceItemId,
				);
				return {
					description: invoiceItem?.description || "N/A",
					type:
						itemTypeLabels[invoiceItem?.type as string] ||
						invoiceItem?.type ||
						"",
			...
			vatExemptText:
				receipt.invoice.company.exemptVat &&
				(receipt.invoice.company.country || "").toUpperCase() === "FRANCE"
	...
	async sendReceiptByEmail(id: string) {
		...
		if (!receipt || !receipt.invoice || !receipt.invoice.client) {
			...
			throw new BadRequestException(
				"Receipt or associated invoice/client not found",
			);
		}
		...
		if (!mailTemplate) {
			logger.error("Email template for receipt not found.", {
				category: "receipt",
			});
		...
		if (!receipt.invoice.client.contactEmail) {
			...
			throw new BadRequestException(
				"Client has no email configured; receipt not sent",
			);
		}
