...
describe('PdfLinksService', () => {
    ...
    it('creates a token record scoped to the company/document and returns the raw token', async () => {
        ...
        expect(prisma.pdfDownloadToken.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('resolves a token to its record when not expired', async () => {
        const record = { documentType: 'QUOTE', documentId: 'q1', companyId: 'company1', expiresAt: new Date(Date.now() + 60_000) };
