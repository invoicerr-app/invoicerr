      ...
      'cac:AccountingSupplierParty': {
        'cac:Party': {
          ...
          'cac:PostalAddress': {
            ...
            'cac:Country': {
              'cbc:IdentificationCode': (data.company.country || 'SA').slice(0, 2).toUpperCase(),
            },
          },
      ...
      'cac:AccountingCustomerParty': {
        'cac:Party': {
          ...
          'cac:PostalAddress': {
            ...
            'cac:Country': {
              'cbc:IdentificationCode': (data.client.country || '').slice(0, 2).toUpperCase(),
            },
          },
          'cac:PartyLegalEntity': {
      ...
      'cac:TaxTotal': [
        {
          ...
          'cac:TaxSubtotal': Object.values(
            ...
            'cac:TaxCategory': {
              ...
              'cbc:Percent': String(
                Object.keys(
                  data.items.reduce<Record<number, boolean>>((a, i) => {
                    a[i.vatRate || 0] = true;
                    return a;
                  }, {}),
                ).find((r) => Math.abs((parseFloat(r) * g.taxable) / 100 - g.tax) < 0.01) || 0,
              ),
