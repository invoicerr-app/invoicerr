...
export class DangerController {
  ...
  @Post('reset/app')
  @ApiOperation({ summary: 'Reset app data', description: 'Deletes all documents (invoices, quotes, payments) for the active company while preserving its configuration.' })
  ...
  @Post('reset/all')
  @ApiOperation({ summary: 'Reset everything', description: 'Deletes all data including documents, clients, and configuration for the active company. The company returns to its initial state.' })
  @ApiQuery({ name: 'otp', required: true, type: String, description: 'One-time passcode sent via POST /danger/otp' })
