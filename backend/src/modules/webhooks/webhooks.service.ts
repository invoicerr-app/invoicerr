...
export class WebhooksService {
	 ...
	 */
	async handlePluginWebhook(
		pluginId: string,
		body: unknown,
		req: Request,
	): Promise<unknown> {
		logger.info(`Processing webhook for plugin: ${pluginId}`, {
			category: "webhook",
			details: { pluginId },
		});
		...
		if (!plugin) {
			logger.warn(
				...
				{ category: "webhook", details: { pluginId } },
			);
		...
		const provider =
			await this.pluginsService.getProviderByType<IWebhookProvider>(
		...
		if (!provider) {
			...
			throw new NotFoundException(
				`No provider found for plugin type: ${plugin.type}`,
			);
		}
		...
		if (typeof provider.handleWebhook !== "function") {
			logger.warn(
				`Provider for plugin ${plugin.name} does not implement handleWebhook method`,
				{ category: "webhook", details: { pluginName: plugin.name } },
			);
		...
		try {
			...
			logger.error(
				`Error in provider webhook handler for plugin ${plugin.name}`,
				{ category: "webhook", details: { pluginName: plugin.name, error } },
			);
			throw error;
		}
	}
	...
	private getDriver(type: WebhookType): WebhookDriver {
		...
		if (!driver) {
			this.logger.warn(
				`No webhook driver found for type: ${type}, using GenericDriver as fallback`,
			);
