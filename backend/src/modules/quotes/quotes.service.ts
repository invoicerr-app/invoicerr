...
export class QuotesService {
  ...
  async getQuotesTable(
    companyId: string,
    filters: { clientId?: string; year?: string; month?: string; sort?: 'asc' | 'desc' },
  ) {
        ...
        items: {
          create: items.map((item, i) => ({
            ...
            discountAmountMinor: item.discountAmount
              ? toMinor(item.discountAmount, body.currency || client.currency || company.currency)
              : null,
            chargeAmount: item.chargeAmount ?? null,
            chargeAmountMinor: item.chargeAmount
              ? toMinor(item.chargeAmount, body.currency || client.currency || company.currency)
              : null,
  ...
  async editQuote(companyId: string, body: EditQuotesDto) {
    ...
    if (existingQuote.status !== 'DRAFT') {
      logger.error('Only DRAFT quotes can be edited', {
        category: 'quote',
        details: { id, status: existingQuote.status },
      });
            ...
            .map(({ i, originalIdx }) => ({
              ...
              data: {
                ...
                unitPriceMinor: toMinor(
                  i.unitPrice,
                  body.currency || client.currency || company?.currency || 'EUR',
                ),
                ...
                discountAmountMinor: i.discountAmount
                  ? toMinor(i.discountAmount, body.currency || client.currency || company?.currency || 'EUR')
                  : null,
                chargeAmount: i.chargeAmount ?? null,
                chargeAmountMinor: i.chargeAmount
                  ? toMinor(i.chargeAmount, body.currency || client.currency || company?.currency || 'EUR')
                  : null,
  ...
  async deleteQuote(companyId: string, id: string) {
    ...
    if (existingQuote.status !== 'DRAFT') {
      logger.error('Only DRAFT quotes can be deleted', {
        category: 'quote',
        details: { id, status: existingQuote.status },
      });
  ...
  async getQuotePdf(id: string, companyId?: string): Promise<Uint8Array> {
    ...
    if (quote.status === 'SIGNED') {
      ...
      try {
        ...
        logger.error(`Error generating PDF via signing provider, falling back to built-in PDF generation`, {
          category: 'quote',
          details: { error },
        });
      }
  ...
  async markQuoteAsSigned(id: string) {
    ...
    if (existingQuote.status !== 'DRAFT') {
      logger.error('Only DRAFT quotes can be signed', {
        category: 'quote',
        details: { id, status: existingQuote.status },
      });
    ...
    try {
      const complianceCtx: TransactionContext = {
        supplier: {
          ...
          countryCode:
            existingQuote.company.countryCode ?? guessCountryCode(existingQuote.company.country) ?? 'FR',
          role: 'B2B',
          identifiers:
            (existingQuote.company as any).partyIdentifiers?.map((pi: any) => ({
              scheme: pi.scheme,
              value: pi.value,
            })) ?? [],
        },
        buyer: {
          ...
          countryCode:
            existingQuote.client.countryCode ?? guessCountryCode(existingQuote.client.country) ?? 'FR',
          role: existingQuote.client.type === 'INDIVIDUAL' ? 'B2C' : 'B2B',
          identifiers:
            (existingQuote.client as any).partyIdentifiers?.map((pi: any) => ({
              scheme: pi.scheme,
              value: pi.value,
            })) ?? [],
        },
      ...
      logger.warn('ComplianceService.issue failed for quote (non-blocking)', {
        category: 'quote',
    ...
    try {
      ...
      if (uploadedUrls.length > 0) {
        logger.info(`Quote ${id} successfully uploaded to ${uploadedUrls.length} storage provider(s)`, {
          category: 'quote',
