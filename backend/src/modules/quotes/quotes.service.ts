...
export class QuotesService {
	...
	async editQuote(body: EditQuotesDto) {
		...
		const updateQuote = await prisma.quote.update({
			...
			data: {
				...
				paymentMethodId:
					(data as any).paymentMethodId || existingQuote.paymentMethodId,
				totalHT,
				totalVAT,
				totalTTC,
				items: {
		...
		logger.info("Quote updated", {
			category: "quote",
			details: { quoteId: updateQuote.id },
		});
	...
	async getQuotePdf(id: string): Promise<Uint8Array> {
		...
		const html = template({
			...
			vatExemptText:
				quote.company.exemptVat &&
				(quote.company.country || "").toUpperCase() === "FRANCE"
	...
	async markQuoteAsSigned(id: string) {
		...
		logger.info("Quote marked as signed", {
			category: "quote",
			details: { quoteId: id },
		});
		...
		try {
			logger.info(`Uploading signed quote ${id} to storage providers...`, {
				category: "quote",
			});
			const pdfBuffer = await this.getQuotePdf(id);
			const uploadedUrls = await StorageUploadService.uploadSignedQuotePdf(
				id,
				pdfBuffer,
			);
