...
export class PaymentsService {
  ...
  async createPayment(body: CreatePaymentDto) {
    ...
    const payment = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      const assignment = await this.numberingService.nextNumber(tx, invoice.companyId, 'payment', paidAtDate);
    ...
    logger.info('Payment created', {
      category: 'payment',
      details: { paymentId: payment.id, companyId: invoice.company?.id },
    });
  ...
  async createPaymentFromInvoice(
    ...
    await this.dispatchPaymentEvent(
    ...
    logger.info('Payment created from invoice', {
      category: 'payment',
      details: { paymentId: newPayment.id, invoiceId },
    });
  ...
  async getPaymentPdf(paymentId: string): Promise<Uint8Array> {
    ...
    if (payment.invoice.client.name.length === 0) {
      payment.invoice.client.name =
        payment.invoice.client.contactFirstname + ' ' + payment.invoice.client.contactLastname;
    }
    ...
    const html = template({
      ...
      vatExemptText:
        payment.invoice.company.exemptVat &&
        (payment.invoice.company.country || '').toUpperCase() === 'FRANCE'
  ...
  async sendPaymentByEmail(id: string) {
    ...
    if (!payment || !payment.invoice || !payment.invoice.client) {
      logger.error('Payment or associated invoice/client not found', {
        category: 'payment',
        details: { id },
      });
    ...
    if (!payment.invoice.client.contactEmail) {
      logger.error('Client has no email configured; payment not sent', {
        category: 'payment',
        details: { id },
      });
