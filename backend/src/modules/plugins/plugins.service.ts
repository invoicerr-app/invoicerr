...
export interface IPlugin {
	...
	description: string;
	init?: () => void;
	// biome-ignore lint/suspicious/noExplicitAny: Plugin config structure varies by plugin
	config?: any;
	type?: string;
	isActive?: boolean;
}
...
export class PluginsService {
	...
	async cloneRepo(gitUrl: string, name: string): Promise<string> {
		...
		if (!existsSync(pluginPath)) {
			logger.info(`Cloning plugin "${name}" from ${gitUrl}...`, {
				category: "plugin",
			});
	...
	async toggleInAppPlugin(id: string) {
		...
		if (!plugin) {
			logger.error(`Plugin with id "${id}" not found`, {
				category: "plugin",
				details: { id },
			});
		...
		if (
			existingActivePlugin &&
			!PluginRegistry.multiInstancePluginTypes.has(plugin.type)
		) {
			logger.error(
	...
	async configureInAppPlugin(id: string, config: Record<string, unknown>) {
		const plugin = await prisma.plugin.findFirst({
			where: { id },
		});
	...
	async getActivePlugin(id: string): Promise<IPlugin | null> {
		...
		const activePlugin = await prisma.plugin.findFirst({
			where: {
				// biome-ignore lint/suspicious/noExplicitAny: Prisma enum type cast
	...
	async generateXml(format: string, _xmlInvoice: unknown): Promise<string> {
		...
		throw new Error(
			`XML generation for format "${format}" not implemented yet`,
		);
	}
	...
	async deletePlugin(uuid: string): Promise<boolean> {
		...
		if (index === -1) {
			logger.error(`Plugin with UUID "${uuid}" not found`, {
				category: "plugin",
				details: { uuid },
			});
	 ...
	 */
	async pluginValidation(pluginId: string): Promise<{
		webhookUrl?: string;
		...
		if (!plugin) {
			...
			throw new BadRequestException(
				`Active plugin with id "${pluginId}" not found`,
			);
		}
		...
		// biome-ignore lint/suspicious/noExplicitAny: Plugin provider type is dynamic
		const provider = await this.pluginRegistry.getProvider<any>(
			plugin.type.toLowerCase(),
		);
		...
		if (provider && typeof provider.handleWebhook === "function") {
			logger.info(
				`Plugin ${plugin.name} supports webhooks (handleWebhook method found)`,
				{ category: "plugin", details: { pluginName: plugin.name } },
			);
			...
			logger.info(
				`Generated webhook URL for plugin ${plugin.name}: ${webhookUrl}`,
				{
					category: "plugin",
			...
			logger.info(
				`Plugin ${plugin.name} does not support webhooks (handleWebhook method not found)`,
				{ category: "plugin", details: { pluginName: plugin.name } },
			);
		...
		if (provider && typeof provider.validatePlugin === "function") {
			try {
				...
				logger.info(
					`Plugin ${plugin.name} validated successfully by provider`,
					{ category: "plugin", details: { pluginName: plugin.name } },
				);
			} catch (error) {
				...
				throw new BadRequestException(
					`Plugin validation failed: ${error.message}`,
				);
			}
		}
	 ...
	 */
	private generatePluginInstructions(
		// biome-ignore lint/suspicious/noExplicitAny: Plugin type is dynamic
		plugin: any,
		webhookUrl?: string,
		webhookSecret?: string,
	): string[] {
		...
		instructions.forEach((instruction) =>
			logger.info(instruction, { category: "plugin" }),
		);
