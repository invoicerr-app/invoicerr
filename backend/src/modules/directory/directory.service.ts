...
export class DirectoryService {
	...
	async listDirectories(basePath: string = "/"): Promise<IDirectoryItem[]> {
		try {
			...
			if (!existsSync(normalizedPath)) {
				logger.error(`Path does not exist: ${normalizedPath}`, {
					category: "directory",
			...
			logger.error(
				...
				{ category: "directory", details: { error } },
			);
			throw new BadRequestException(
				`Failed to list directories: ${error instanceof Error ? error.message : String(error)}`,
			);
	...
	async getDirectoryInfo(basePath: string = "/"): Promise<{
		...
		const parentPath =
			normalizedPath === "/" ? null : resolve(normalizedPath, "..");
	...
	private validatePath(path: string): void {
		...
		if (!isAllowed) {
			logger.error(
				...
				{
					category: "directory",
					details: { path: normalizedPath, allowedRoots: this.allowedRoots },
				},
			);
			throw new BadRequestException(
				`Access denied. Path is not within allowed directories. Allowed roots: ${this.allowedRoots.join(", ")}`,
	...
	async createDirectory(
		...
		try {
			...
			if (!existsSync(normalizedParent)) {
				logger.error(`Parent path does not exist: ${normalizedParent}`, {
					category: "directory",
					details: { parentPath: normalizedParent },
				});
				throw new BadRequestException(
					`Parent path does not exist: ${normalizedParent}`,
				);
			}

			const parentStats = statSync(normalizedParent);
			if (!parentStats.isDirectory()) {
				logger.error(`Parent path is not a directory: ${normalizedParent}`, {
					category: "directory",
					details: { parentPath: normalizedParent },
				});
				throw new BadRequestException(
					`Parent path is not a directory: ${normalizedParent}`,
				);
			}
			...
			if (folderName.includes("/") || folderName.includes("\\")) {
				logger.error("Folder name cannot contain path separators", {
					category: "directory",
				});
				throw new BadRequestException(
			...
			if (folderName.includes("..")) {
				logger.error("Folder name cannot contain parent directory references", {
					category: "directory",
				});
				throw new BadRequestException(
					"Folder name cannot contain parent directory references",
				);
			}
			...
			if (existsSync(newPath)) {
				logger.error(`Folder already exists: ${newPath}`, {
					category: "directory",
