 ...
 * `SemanticInvoiceInput.customizationId`'s own header. `options.businessProcessCodeOverride` has THREE
 * callers today: the Chorus Pro-configured `facturx-provider.ts` instance (its own, Chorus-specific
 * "Cadre de facturation" vocabulary), `peppol-bis-provider.ts` (the fixed Peppol BIS Billing
 * profile URN, GH-448) and `xrechnung-provider.ts` (the Default-Wert XRechnung 3.0.2 §11.27 names for
 * that same element, XRechnung 3.0.2 section 11.27). See `SemanticInvoiceInput.businessProcessCodeOverride`'s own header
 * for all three.
