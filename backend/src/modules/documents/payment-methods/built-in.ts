...
import { molliePaymentMethod } from './mollie.descriptor';
 ...
 * never drift apart (one list, read twice). Adding a method is exactly one more entry here plus its
 * own descriptor file — no controller change, no frontend screen change (both only ever iterate
 * whatever this list (via the registry) contains). "mollie" (2026-09-15, alongside the real Mollie
 * `PaymentProvider`) is the same trivial "accepts this provider" marker `stripePaymentMethod` already
 * is — see `mollie.descriptor.ts`'s own header.
 *
