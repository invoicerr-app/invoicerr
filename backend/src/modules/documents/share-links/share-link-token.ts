 ...
 * own. Same discipline as `@/utils/api-key.ts`'s `hashApiKey`, and the removed compliance engine's
 * own pdf-links.service.ts (git tag `avant-refonte-documents`) before it: no hand-rolled crypto,
 * node's native `crypto` module only.
 *
 ...
 * no timing side-channel to close with a constant-time compare either (the removed compliance
 * engine's own service carried the identical reasoning in its header, and never added one). A fast
 * digest (SHA-256) is exactly right here: it is not protecting against an attacker who already has
 * the hash and wants to invert it (there is nothing to invert — the entropy lives in the random
 * token, not the hash), only turning an unbounded-length secret into a fixed-size, indexable
 * lookup key.
 */
import { createHash, randomBytes } from 'node:crypto';

/** 256 bits — the same entropy budget the removed compliance engine's own `PdfLinksService` used
