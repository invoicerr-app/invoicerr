/**
 * REAL round-trip against the FACe SSPP sandbox.
 *
 ...
 *
 * HONEST STATUS: the credential-gated `describeLive` block (needing a
 * FACe-registered PKCS#12) is **skipped, always** — this checkout holds no such certificate (a
 * FNMT/representation certificate is a human, interactive procedure — see `CREDENTIALS_GUIDE.md`'s
 * own FACe section). But the CREDENTIAL-FREE `describeReachability` block below proves something
 * load-bearing: a WS-Security-signed envelope, signed with nothing more than a throwaway
 * in-memory self-signed test certificate, makes the live sandbox's OWN fault CHANGE NATURE.
 *
 * ## WHAT WAS LIVE-VERIFIED, 2026-09-02 — the raw evidence, both ways
 *
 ...
 * vs. "error validating the certificate"). This proves FACe's
 ...
 *
 * INCIDENTAL FINDING: the live sandbox's OWN
 ...
 * narrow but real: the transport DOES sign the envelope, but NO
 ...
 * demonstrates without needing any credential at all. The gated block's own assertion reflects this:
 * it does not pin the outcome to "always rejects with /firmad/i" (that fault no longer occurs once
 * the envelope is signed) — a genuinely FACe-registered certificate might actually succeed, which this
...
describeReachability('FACe SSPP — credential-free reachability proof', () => {
  it('the real sandbox is reachable and answers an UNSIGNED request with a genuine SOAP Fault naming the WS-Security gap', async () => {
    ...
    // observed BOTH 500 and 200 for the IDENTICAL fault body (a load-balanced
  ...
  // THE CORE PROOF — see this file's own header for the raw evidence quoted
  ...
  it('a WS-Security-SIGNED request (throwaway test cert) makes the fault change nature — from "not signed" to a certificate-validation error', async () => {
    ...
    // legitimately drift) — but the live run got exactly "Error al validar el
...
describeLive(
  ...
  () => {
    it('a real enviarFactura call, WS-Security-signed with the connected certificate, never produces a SILENT/FAKE result either way', async () => {
      ...
      // an open question that cannot be settled without one. If this checkout ever
