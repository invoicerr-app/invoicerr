...
describe('buildPeppolTransport', () => {
  ...
  describe('send() — THE FORMAT OVERRIDE (a B2G rule imposing a different CONTENT over the same Peppol channel)', () => {
    ...
    // each of THOSE transports' own specs already prove (their own `send()` never even accepts this
    // field). The contract itself (`transport-registry.ts`'s own header) is documentation, not a
