 ...
 * instead — see persistence.ts's own `dateValueInRange`). Not scoped to one document, unlike
 ...
 * contribution (contributions/invoice-contributions.ts), which reads this for every sent invoice of
 * the company at once. A document with no payments at all is simply ABSENT
