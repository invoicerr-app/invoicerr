      ...
      for (const recurringInvoice of recurringInvoices) {
        try {
          ...
          if (recurringInvoice.skipNext) {
            ...
            this.logger.log(
              `Skipped next cycle (${skippedDate.toISOString()}) for recurring ${recurringInvoice.id}`,
            );
          }
          ...
          while (cycleDate <= today) {
            ...
            try {
              ...
              this.logger.log(
                `Generated invoice ${invoice.id} for recurring ${recurringInvoice.id} (period: ${periodKey}, planned: ${cycleDate.toISOString()})`,
              );
            } catch (error: any) {
              // P2002 = Prisma unique constraint violation = cycle already generated
              if (error?.code === 'P2002') {
                this.logger.log(
                  `Period ${periodKey} already generated for recurring ${recurringInvoice.id}, skipping`,
                );
              } else {
                this.logger.error(
                  `Error generating invoice for recurring ${recurringInvoice.id} (period: ${periodKey}):`,
                  error,
                );
