...
export const getQuotePdfTool: ToolDescriptor<typeof inputSchema> = {
    ...
    handler: async (ctx, input) => {
        ...
        return {
            content: [
                { type: 'text', text: `Quote ${input.quoteId} PDF fetched (${pdfBytes.length} bytes). Download link (valid 1 hour): ${downloadUrl}` },
                { type: 'resource', resource: { uri: `invoicerr://quotes/${input.quoteId}.pdf`, mimeType: 'application/pdf', blob } },
            ],
            structuredContent: { quoteId: input.quoteId, mimeType: 'application/pdf', sizeBytes: pdfBytes.length, downloadUrl },
        };
    },
};
