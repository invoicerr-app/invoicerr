...
const inputSchema = {
    ...
    items: z.array(z.object({
        ...
        quantity: z.number().describe('Quantity to invoice for this quote item — must not exceed the remaining invoicable quantity'),
    })).min(1),
};
...
export const createInvoiceFromQuoteTool: ToolDescriptor<typeof inputSchema> = {
    ...
    description: 'Create an invoice from an existing quote, for a partial or full selection of the quote\'s items.',
    ...
    handler: async (ctx, input) => {
        ...
        return {
            content: [{ type: 'text', text: `Invoice ${invoice.rawNumber || invoice.number} created from quote (id: ${invoice.id}).` }],
