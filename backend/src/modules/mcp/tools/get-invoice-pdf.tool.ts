  ...
  handler: async (ctx, input) => {
    ...
    return {
      content: [
        {
          type: 'text',
          text: `Invoice ${input.invoiceId} PDF fetched (${pdfBytes.length} bytes). Download link (valid 1 hour): ${downloadUrl}`,
        },
        {
          type: 'resource',
          resource: { uri: `invoicerr://invoices/${input.invoiceId}.pdf`, mimeType: 'application/pdf', blob },
        },
      ],
      structuredContent: {
