...
/** VAT-ish keywords across the six target languages (+ Spanish/Portuguese `NIF`, thrown
...
/** Keyword sets across the six target languages. Checked in this exact priority order per
 ...
 *  MONTH-DAY-YEAR): the six target languages are all day-first locales; a US-style
...
/** Issuance-related keywords across the six target languages, used only to PREFER a date
...
/** Document-title words the six target languages use for "invoice"/"quote" — a keyword
...
export function mapOcrTextToProposal(text: string): ExtractedInvoiceProposal {
  ...
  // fields it targets (amounts/date/number/VAT id/supplier name). Per-line
