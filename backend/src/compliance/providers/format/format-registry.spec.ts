...
import type {
	CompliancePlan,
	PlannedArtifact,
} from "../../engine/compliance-engine";
import { RecordingComplianceLogger } from "../../execution/logger";
import type { DocumentSyntax } from "../../types";
import type {
	InvoiceArtifactPort,
	XmlExportFormat,
} from "./invoice-artifact-port";
...
describe("FormatProviderRegistry resolution", () => {
	...
	it("resolves bespoke + generic providers", () => {
		...
		expect(defaultFormatRegistry.resolve("FATTURAPA")?.id).toBe(
			"fatturapa-1.2",
		);
		expect(defaultFormatRegistry.resolve("KSA_UBL")?.id).toBe("ksa-ubl");
		expect(defaultFormatRegistry.resolve("FA_VAT")?.id).toBe("fa-vat");
		expect(defaultFormatRegistry.resolve("ES_FACTURAE")?.id).toBe(
			"es-facturae",
		);
		expect(defaultFormatRegistry.resolve("NATIONAL_XML")?.id).toBe(
			"national-xml",
		);
	});
	...
	it("an unknown syntax resolves to null", () => {
		expect(
			defaultFormatRegistry.resolve("TOTALLY_UNKNOWN" as DocumentSyntax),
		).toBeNull();
	});
...
describe("Reachability: executor → provider → port", () => {
	for (const { syntax, label } of REACHABILITY_CASES) {
		it(`${label} (${syntax}) — registry.buildAll emits XML from port, not empty stub`, async () => {
			const registry = new FormatProviderRegistry({
				artifacts: reachablePort(),
			});
			const plan = {
				artifacts: [{ role: "AUTHORITATIVE", syntax }],
			} as CompliancePlan;
	...
	it("NATIONAL_XML routes by countryCode via port.renderNationalXml(invoiceId, cc)", async () => {
		...
		const port: InvoiceArtifactPort = {
			...reachablePort(),
			renderNationalXml: async (_id: string, cc: string) => {
				receivedCc = cc;
				return MARKER_XML;
			},
		};
		...
		const plan = {
			artifacts: [{ role: "AUTHORITATIVE", syntax: "NATIONAL_XML" }],
		} as CompliancePlan;
	...
	it("without port, all national providers fall back to empty stub (no crash)", async () => {
		...
		const plan = {
			artifacts: [{ role: "AUTHORITATIVE", syntax: "CFDI" }],
		} as CompliancePlan;
