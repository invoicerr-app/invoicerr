...
export class SriTransmissionProvider implements TransmissionProvider {
    ...
    fields: [
      {
        type: 'select',
        name: 'environment',
        label: 'SRI environment',
        required: true,
        options: [
          { label: 'Pruebas (test)', value: 'test' },
      ...
      { type: 'text', name: 'ruc', label: 'RUC (13 chars)', placeholder: '1792345678001', required: true },
      {
        type: 'text',
        name: 'claveAcceso',
        label: 'Clave de Acceso (49 chars — set by transmission)',
        required: false,
      },
      {
        type: 'text',
  ...
  async transmit(
    ...
    if (!ecArtifact)
      return { channel: 'GOV_PORTAL_API', status: 'SKIPPED', notes: ['sri: no EC_FE artifact'] };
    const companyId = ctx.supplierCompanyId;
    if (!companyId)
      return { channel: 'GOV_PORTAL_API', status: 'SKIPPED', notes: ['sri: no supplierCompanyId'] };

    const http = this.httpPort ?? buildStub();
    try {
      const client = new SriClient(http, {
        environment: env,
        ruc,
      ...
      const xmlBytes = Buffer.isBuffer(ecArtifact.bytes) ? ecArtifact.bytes : Buffer.from(ecArtifact.bytes);
      ...
      if (SriClient.mapEstado(recepResp.estado) === 'REJECTED') {
        ...
        return {
          ...
          status: 'REJECTED',
          notes: [`sri: ${recepResp.estado} — ${errs}`],
        };
      }
  ...
  async poll(ref: string, log: ComplianceLogger): Promise<TransmissionResult> {
    ...
    if (parts.length !== 2)
      return { channel: 'GOV_PORTAL_API', status: 'PENDING', ref, notes: ['sri: invalid ref'] };
    ...
    try {
      ...
      if (!resolved?.isActive)
        return { channel: 'GOV_PORTAL_API', status: 'PENDING', ref, notes: ['sri: credentials inactive'] };
      const { config, environment } = resolved;
      const env = ((config.environment as string) ?? environment ?? 'test').toLowerCase() as 'test' | 'prod';
      const http = this.httpPort ?? buildStub();
      const client = new SriClient(http, {
        environment: env,
        ruc: config.ruc as string,
        certBase64: config.certBase64 as string | undefined,
        certPassword: config.certPassword as string | undefined,
      });
      ...
      if (!aut)
        return {
          ...
          status: 'PENDING',
          ref,
          notes: ['sri: no autorizacion in response'],
        };
      if (aut.estado === 'AUTORIZADO') {
        return {
          ...
          status: 'CLEARED',
          ref,
          authorityIds: aut.numeroAutorizacion
            ? [{ scheme: 'AUTORIZACION', value: aut.numeroAutorizacion }]
            : [],
      ...
      return {
        ...
        status: 'REJECTED',
        ref,
        notes: [`sri: NO AUTORIZADO — ${errMsg}`],
      };
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
...
function buildStub(): SriHttpPort {
  return {
    recibirComprobante: async () => {
      throw new Error('SriHttpPort not implemented — SRI cert required');
    },
    autorizarComprobante: async () => {
      throw new Error('SriHttpPort not implemented');
    },
  };
}
