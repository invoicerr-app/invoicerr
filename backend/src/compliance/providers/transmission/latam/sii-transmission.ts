...
export class SiiTransmissionProvider implements TransmissionProvider {
  ...
  /** IMPLEMENTED — real seed→token→EnvioDTE→poll scaffold, awaiting a certification-env cert. */
    ...
    fields: [
      {
        type: 'select',
        name: 'environment',
        label: 'SII environment',
        required: true,
        options: [
          { label: 'Certificación (test)', value: 'cert' },
      ...
      },
      {
        type: 'text',
        name: 'rut',
        label: 'RUT (digits only, without DV)',
  ...
  async transmit(
    ...
    try {
      const client = new SiiClient(http, {
        environment: env,
        rut,
        dv,
    ...
    try {
      ...
      if (!resolved?.isActive) {
        return {
          ...
          status: 'PENDING',
          ref,
          notes: ['sii: credentials no longer active'],
        };
      }
      ...
      if (cat === 'REJECTED')
        return {
          ...
          status: 'REJECTED',
          ref,
          notes: [`sii: ${resp.estado}: ${resp.glosa ?? ''}`],
        };
      return {
        channel: 'GOV_PORTAL_API',
        status: 'PENDING',
...
function buildStub(): SiiHttpPort {
  return {
    getSeed: async () => {
      throw new Error('SiiHttpPort not implemented — SII cert required');
    },
    getToken: async () => {
      throw new Error('SiiHttpPort not implemented');
    },
    submitEnvioDTE: async () => {
      throw new Error('SiiHttpPort not implemented');
    },
    queryEstado: async () => {
      throw new Error('SiiHttpPort not implemented');
