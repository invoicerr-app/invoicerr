...
export class PostAuditRegimeHandler implements RegimeHandler {
  ...
  handle(): RegimeResult {
    return {
      ...
      cleared: true,
      authorityIds: [],
      notes: ['post-audit: no real-time obligation'],
    };
  }
}
...
export class PeriodicReportingRegimeHandler implements RegimeHandler {
  ...
  handle(
    ...
    return {
      ...
      cleared: true,
      authorityIds: [],
      notes: ['queued for periodic reporting'],
    };
  }
}
...
export class RealTimeReportingRegimeHandler implements RegimeHandler {
  ...
  handle(
    ...
    log.todo(
      'regime/real-time-reporting',
      'push transaction data to the authority within the mandated window',
    );
    return {
      model: this.model,
      clearanceRequired: false,
...
export class ClearanceRegimeHandler implements RegimeHandler {
  ...
  handle(
    ...
    log.todo(
      'regime/clearance',
      'submit for clearance and await authorisation (UUID/folio/protocol) before the invoice is valid',
    );
    ...
    return {
      ...
      cleared: false,
      authorityIds: [],
      notes: ['awaiting clearance (async)'],
    };
  }
}
...
export class DecentralizedCtcRegimeHandler implements RegimeHandler {
  ...
  handle(
    ...
    log.todo(
      'regime/decentralized-ctc',
      'route via PDP/Peppol + extract e-reporting; track lifecycle statuses',
    );
    return {
      model: this.model,
      clearanceRequired: false,
      cleared: true,
      authorityIds: [],
      notes: ['routed via decentralized CTC network'],
    };
  }
}
