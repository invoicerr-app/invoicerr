...
describe('InboundRouter — register / receive / dedup / correlate', () => {
  ...
  it('routes a matching inbound message into the document runtime as INBOUND_STATUS', async () => {
    ...
    const res = await router.receive({
      channel: 'SDI',
      correlationKey: 'ref1',
      status: 'consegnata',
      rawRef: 'm1',
    ...
    expect(res).toEqual({
      kind: 'ROUTED',
      documentId: 'doc1',
      signal: { type: 'INBOUND_STATUS', status: 'consegnata' },
    });
  ...
  it('drops a duplicate (same provider ref) without re-applying', async () => {
    ...
    const dup = await router.receive({
      channel: 'SDI',
      correlationKey: 'ref1',
      status: 'consegnata',
      rawRef: 'm1',
...
describe('InboundRouter × LifecycleRuntime — pushed statuses drive the lifecycle', () => {
  ...
  const graphOf = (s: string, b: string, sup: SupplyType, d: string) =>
    assembleFromPlan(resolve(tx(s, b, 'B2B', sup, d)));

  it('IT (SdI, callback): an authority "consegnata" push drives PENDING_CLEARANCE → CLEARED', async () => {
    const runtime = new LifecycleRuntime(
      graphOf('IT', 'IT', 'GOODS', '2027-01-15'),
      'ISSUED',
  ...
  it('FR (PDP, callback): an "approuvée" push drives AWAITING_RESPONSE → ACCEPTED', async () => {
    const runtime = new LifecycleRuntime(
      graphOf('FR', 'FR', 'SERVICES', '2027-01-15'),
      'DELIVERED',
      new RecordingComplianceLogger(),
    );
  ...
  it('FR (PDP, callback): a "refusée" push drives AWAITING_RESPONSE → REFUSED', async () => {
    ...
    await router.register(
      ...
      { channel: 'PDP', correlationKey: 'pdp-2' },
    );
