#!/usr/bin/env node
/**
 * i18n consistency checker — the backend's own copy of `frontend/scripts/i18n-check.mjs`'s logic,
 * pointed at `src/mail` (the only place this project's `mailT()`/`t()` calls live — see
 * `src/mail/i18n.ts`) instead of the frontend's whole `src` tree and `translation.json` catalog.
 *
 * WHY A COPY, NOT AN IMPORT of the frontend's script: this repository deliberately has no npm
 * workspace root — `backend/`, `frontend/`, `e2e/` and `documentation/` are four independent
 * projects, each with its own `package.json`/`node_modules` (see this repo's own root guidance). A
 * relative `import` here reaching into `../../frontend/scripts/…` would work on THIS checkout by
 * accident of both projects sharing one git repo, but breaks the moment either project is built,
 * packaged or extracted on its own (the backend's Docker image, a `git worktree` of just this
 * directory, …) — exactly the coupling that split existing in the first place. The two scripts share
 * an algorithm, not a runtime dependency; keeping them as two files with the same logic, kept in sync
 * by hand, costs less than inventing a shared package for a single dev-time lint script.
 *
 ...
 *
 * Flags — same shape as the frontend's own script, defaulted to this project's layout:
 *   --src <dir>             directory to scan for t() calls, relative to backend/ (default:
 *                           "src/mail" — the one namespace this catalog covers)
 *   --locales <dir>         directory holding one subfolder per language, relative to backend/
 *                           (default: "src/mail/locales")
 *   --file <name>           catalog filename inside each language's subfolder (default: "mails.json")
 ...
 *                           it (repeatable) — none needed today, unlike the frontend's catalog.
...
//    (keys are sometimes stored in data structures and passed to t() later). Expected to catch, for
//    instance, this catalog's own plural `_one` variants (i18next resolves those from the BARE key
//    called in code, e.g. `demo.itemCount`, never from the suffixed one) — a warning, never a failure,
//    exactly like the frontend catalog's own already-shipped plural keys.
...
for (const locale of locales) {
  const data = flatten(
    JSON.parse(fs.readFileSync(path.join(LOCALES_DIR, locale, CATALOG_FILE_NAME), 'utf8')),
  );
...
if (REPORT) {
  ...
  console.log(
    `i18n-check: ${staticKeys.size} static keys used, ${dynamicPatterns.size} dynamic patterns, ${enKeys.size} keys defined in ${sourceLabel}`,
  );
  ...
  for (const [locale, c] of Object.entries(coverage)) {
    console.log(
      `  ${locale.padEnd(8)} ${String(c.pct).padStart(5)}%  (${c.present}/${c.total}${c.extraKeys ? `, ${c.extraKeys} extra` : ''})`,
    );
  }
}
