...
La plupart des points de terminaison nécessitent soit un cookie de session (connexion via le navigateur), soit une clé API. Voir [Authentification](authentication.md) pour le détail des deux mécanismes, et le module `api-keys` pour gérer les clés depuis l'interface.
