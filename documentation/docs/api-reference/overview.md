...
Most endpoints require either a session cookie (browser login) or an API key. See [Authentication](../developer-guide/authentication.md) for details on both mechanisms, and the `api-keys` module for managing keys from the UI.
