...
the contract each file has to satisfy; this is only the shopping list for Canada.

- `country-policy/data/ca.json` — which document actions Canada allows. Start here: without this
  file every action is refused with a 403, naming the country.
- `country-identifiers/data/ca.json` — which national identifier a party must carry.
- `tax/tax-systems/data/ca.json` and `vat-rates/data/ca.json` — the tax kind, and the rate ladder
  offered on an invoice line.
- `correction-routes/data/ca.json` — all eleven canonical correction routes, `unverified` where
  unresearched, never omitted.
- `b2g-routing/data/ca.json` — the channel and format a Canadian public buyer requires.
- `transports/channel-policy/data/ca.json` — whether a channel is legally required of a seller
  established in Canada, and from what date.
- `archive/retention/data/ca.json` — how long an archived document is kept, and what the duration is
  counted from.
