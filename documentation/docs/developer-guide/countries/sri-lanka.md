...
the contract each file has to satisfy; this is only the shopping list for Sri Lanka.

- `country-policy/data/lk.json` — which document actions Sri Lanka allows. Start here: without this
  file every action is refused with a 403, naming the country.
- `country-identifiers/data/lk.json` — which national identifier a party must carry.
- `tax/tax-systems/data/lk.json` and `vat-rates/data/lk.json` — the tax kind, and the rate ladder
  offered on an invoice line.
- `correction-routes/data/lk.json` — all eleven canonical correction routes, `unverified` where
  unresearched, never omitted.
- `b2g-routing/data/lk.json` — the channel and format a Sri Lankan public buyer requires.
- `transports/channel-policy/data/lk.json` — whether a channel is legally required of a seller
  established in Sri Lanka, and from what date.
- `archive/retention/data/lk.json` — how long an archived document is kept, and what the duration is
  counted from.
