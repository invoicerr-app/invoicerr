...
the contract each file has to satisfy; this is only the shopping list for Mozambique.

- `country-policy/data/mz.json` — which document actions Mozambique allows. Start here: without this
  file every action is refused with a 403, naming the country.
- `country-identifiers/data/mz.json` — which national identifier a party must carry.
- `tax/tax-systems/data/mz.json` and `vat-rates/data/mz.json` — the tax kind, and the rate ladder
  offered on an invoice line.
- `correction-routes/data/mz.json` — all eleven canonical correction routes, `unverified` where
  unresearched, never omitted.
- `b2g-routing/data/mz.json` — the channel and format a Mozambican public buyer requires.
- `transports/channel-policy/data/mz.json` — whether a channel is legally required of a seller
  established in Mozambique, and from what date.
- `archive/retention/data/mz.json` — how long an archived document is kept, and what the duration is
  counted from.
