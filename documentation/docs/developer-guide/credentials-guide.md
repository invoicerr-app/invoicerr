...
- **Reception endpoint (`SdIRiceviNotifiche`)** — not a secret, but a URL you must supply back TO AdE at Step 3 below, not one AdE hands you: `https://<your instance>/api/public/sdi/notifiche/<pushToken>`, where `<pushToken>` is this company's own `CompanyChannelConfig.pushToken` (`GET /api/company/channels` returns it once "sdi" is connected). Register exactly this per-company URL, never the bare `.../notifiche` path with no token — the token is what lets this backend tell your company's own notifiche apart from every other company's on the same instance (`backend/src/modules/documents/transports/sdi/sdi-notifiche.controller.ts`'s own header has the full reasoning). The bare, un-tokened path is still answered (a company that registered it before this existed keeps working) but trusts a weaker, content-based check instead of the URL itself — never register it for a NEW channel.
...
3. Pick channel type — **SDICoop** (web-service) or **SDIFTP** (SFTP) — and supply the required technical info (service endpoints for reception, if bidirectional): this is where the per-company `.../public/sdi/notifiche/<pushToken>` URL above gets registered as the reception endpoint.
