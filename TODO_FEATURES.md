...
| **Seats** — purchased seats and the open-space page | Owner decision, 2026-09-16 evening: seat quantity is bought in the Polar portal (the app no longer pushes the member count), every member occupies a seat, empty seats are allowed, no break room (no rotation), an invitation is refused when no seat is free, over-capacity parks the most recent members (never the OWNER) on a waiting screen. UI: Settings > Seats, generative open space with drag-and-drop desks plus an equivalent list view. In progress. | `76-seats.cy.ts` — two seats, third invitation refused, move after a third seat. |
