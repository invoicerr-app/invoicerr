...
|---|---|---|
| **UI** — logo integration | Owner picked the series-4 mark (« Faille », 2026-09-16). In progress: favicon (dedicated 16 px redraw), PWA icons (maskable), apple-touch-icon, auth brand mark, sidebar and public-page marks. Form sections by title in the document form would need a grouping field on the backend descriptor — not requested. | None — visual check from screenshots. |
| **Legal** — SaaS legal set and acceptance | Owner decision, 2026-09-16: privacy policy (GDPR), data processing agreement (art. 28), legal notice (LCEN), cookies + acceptable use, all in English next to the existing Terms of Service; hosting provider stays a placeholder. Self-hosted instances accept nothing (AGPL-3.0, no data received); in SaaS mode (billing flag) the sign-up form requires a checkbox, acceptances are stored with the document version, and a newer version triggers re-acceptance at sign-in. In progress. | `75-legal-acceptance.cy.ts` — sign-up with/without the checkbox, re-acceptance on a new version. |
