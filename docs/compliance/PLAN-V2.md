...
| # | Tâche | Dépend de | Accepte si |
| --- | --- | --- | --- |
| **P3-U01** | 🟡 **règle établie, table partielle** — `CORRECTION-JURISDICTION.yaml`. L'art. 219 bis et ses 6 exceptions sont sourcés, l'art. 219 assimile bien le document de correction, et **22 paires sur 49** sont énumérées. Les autres découlent de la même règle mais n'ont pas été sourcées individuellement | — | 🟡 Règle ✅ ; table ✅ pour FR/IT/PL/DE ; ES et MX restent |
...
| 2026-08-29 | **P3-T03** | ✅ fait | L'avoir interne n'est plus transmissible. **Le défaut n'était pas où le plan le situait** : `correctInvoice()` n'enfile aucune transmission — il fabrique un document ORDINAIRE que le prochain clic sur « Envoyer » pousse au PPF. La garde est donc au bon endroit : décision prise à la correction (quand le statut de l'original est certain), consignée en événement `TRANSMISSION_SUPPRESSED` — « nous n'avons pas transmis » est un fait à prouver, pas seulement à faire — puis appliquée par le bouton ET par l'API. **L'Italie en bénéficie sans une ligne de code italienne** : son profil déclare la même voie. Nouvelle garde `data-integrity` : une transmission interdite doit nommer ses statuts, sinon c'est un no-op déguisé en règle. **2029 tests, 0 échec. Et `biome ci` repasse au vert** — il était rouge avant, sur deux fichiers dont un que j'avais restauré à tort en le croyant hors périmètre. |
| 2026-08-29 | **P3-U01** | 🟡 partiel | `CORRECTION-JURISDICTION.yaml`. **L'art. 219 assimile bien le document de correction à une facture**, donc il suit le même rattachement — établi par construction en 4 points, et confirmé par ViDA qui nomme « la facture rectificative visée à l'article 219 ». **Résultat structurant : en B2B transfrontalier c'est presque toujours le pays du FOURNISSEUR** — et vérifié dans le code, notre moteur résout déjà le cycle de vie sur `sp` (profil fournisseur), donc nous avons raison sur le cas dominant. **Mais il y a QUATRE rattachements là où nous n'en avons qu'un** : facturation → fournisseur ; TVA de fond et ses délais → État d'imposition ; canal → établissement ; archivage → hors 219 bis. Deux résultats nets : **KSeF s'applique aux factures vers l'étranger** (remise hors canal avec QR, art. 106gb ust. 4-5) et **le mandat allemand exige les DEUX parties établies** (§ 14 Abs. 2 S. 2 Nr. 1, « und »). Mexique corroboré mais NON établi — le CFF n'a pas de source atteignable. **Deux bombes temporelles au 2027-01-01** : la France abroge CGI 289 et 289-0 sans successeur identifié ; l'Italie abroge tout son bloc facturation mais la règle ne bascule pas, elle est renumérotée. |
