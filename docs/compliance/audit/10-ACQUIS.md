...
> **Corrigé le 2026-08-28.** Deux erreurs dans le paragraphe ci-dessus. Le job `queue-integration`
> de `cypress.yml` **provisionnait déjà** Postgres 16, `DATABASE_URL` et `prisma migrate deploy` :
> il n'y avait pas de Postgres à ajouter, seulement un drapeau à poser. Et ce n'est plus « à
> brancher » — **P1-T06 l'a fait** (`d2207df6`), vérifié dans les deux sens contre Postgres 16.11 :
> 8 tests verts avec le CAS, 2 rouges sans.
