...
| `reconciledPaymentId` peut être mal attribué entre deux lignes | Le paiement produit est identifié par un diff avant/après de `listPayments`, parce que `runAction` ne rend pas l'identifiant créé. Deux lignes rapprochées **sur la même facture** en parallèle peuvent échanger leur référence. Les montants et le solde restent justes ; c'est la piste d'audit qui ment. Confiance moyenne : démontré par la forme du code, pas reproduit. |
| ~~`hiddenReference` (l'`articleId` d'une ligne) n'est pas couvert~~ — FERMÉ (mesuré, pas de code) | L'exposition redoutée (une ligne visant l'article d'une autre société en décrémente le stock) n'existe pas : `ArticlesService.findOne`/`findAll` et `stock/apply-stock-on-issuance.ts` filtrent déjà par le `companyId` de l'AGISSANT (jamais déduit de l'id), donc un `articleId` étranger ne matche nulle part et se comporte comme « pas d'article » — aucun prix, aucune TVA, aucun stock volé. Le rendu PDF exclut en plus ce champ de l'affichage. Reste un trou, mais cosmétique : rien ne refuse l'écriture d'un `articleId` orphelin. Non étendu à `validate-references.ts` : `$rowId` n'est stamped que sur `invoice.lines` (via le `rowSelection` de `credit-note`), jamais sur `quote.lines` — un diff « inchangé vs modifié » par `$rowId` serait fiable pour l'un et faux pour l'autre. Preuve : `stock/apply-stock-on-issuance.spec.ts` (cas multi-société), détail dans `validate-references.ts`. |
| ~~Un champ `reference` n'est pas vérifié à l'écriture~~ — FERMÉ | `descriptors/field-kinds.ts` le dit lui-même : l'existence de la cible n'est « délibérément PAS vérifiée ». Conséquence, une dizaine de `prisma.client.findUnique` non portés par `companyId` (transports, moteur fiscal, rendu) lisent un client d'une autre société si son CUID est deviné. Antérieur aux features d'aujourd'hui, systémique, à traiter d'un bloc. |
