...
| Un champ `reference` n'est pas vérifié à l'écriture | `descriptors/field-kinds.ts` le dit lui-même : l'existence de la cible n'est « délibérément PAS vérifiée ». Conséquence, une dizaine de `prisma.client.findUnique` non portés par `companyId` (transports, moteur fiscal, rendu) lisent un client d'une autre société si son CUID est deviné. Antérieur aux features d'aujourd'hui, systémique, à traiter d'un bloc. |
