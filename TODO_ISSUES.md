...
| `hiddenReference` (l'`articleId` d'une ligne) n'est pas couvert | La passe de cloisonnement a fermé les champs `reference` de premier niveau. L'`articleId` d'une ligne vise le catalogue `article` : question de cloisonnement distincte, non commandée, non fermée. |
