...
export default function SignupPage() {
	...
	const [requiresInvitation, setRequiresInvitation] = useState<boolean | null>(
		null,
	);
	...
	const getEnvVariable = (key: string): string | undefined => {
		return (
			(
				(window as unknown as Record<string, unknown>).__APP_CONFIG__ as Record<
					string,
					unknown
				>
		...
		const checkInvitationRequired = async () => {
			try {
				const response = await fetch(
					`${backendUrl}/api/invitations/is-first-user`,
				);
	...
	const validateInvitationCode = async (
		...
		try {
			...
			if (!response.ok) {
				const data = await response.json();
				throw new Error(
					data.message || t("auth.signup.errors.invalidInvitationCode"),
				);
			}
	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (requiresInvitation) {
			if (!data.invitationCode) {
				setErrors({
					invitationCode: [t("auth.signup.errors.invitationCodeRequired")],
				});
				return;
			}
							...
							<div className="space-y-2">
								...
								<div className="flex items-center gap-2">
									...
									<Input
										...
										name="invitationCode"
										placeholder={t(
											"auth.signup.form.invitationCode.placeholder",
										)}
										disabled={loading}
