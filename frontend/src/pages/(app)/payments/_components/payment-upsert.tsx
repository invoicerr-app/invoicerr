...
export function PaymentUpsert({ payment, open, onOpenChange }: PaymentUpsertDialogProps) {
  ...
  const invoiceList = (Array.isArray(invoices) ? invoices : []).filter(
    (inv) => inv.status !== InvoiceStatus.DRAFT && inv.status !== InvoiceStatus.ARCHIVED,
  )
        ...
        >
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <SearchSelect
                        options={invoiceList.map((invoice) => ({
                          label: invoice.rawNumber || invoice.number?.toString() || invoice.id.slice(0, 8),
                          value: invoice.id,
                        }))}
                        value={field.value ?? ""}
                        onValueChange={(val) => {
                          field.onChange(val || null)
                          const inv = invoiceList.find((inv) => inv.id === val) || null
                          setSelectedInvoice(inv)
                          redistribute(inv, Number(form.getValues("amount")) || 0)
                        }}
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Select value={field.value ?? ""} onValueChange={(val) => field.onChange(val || "")}>
                        <SelectTrigger
                          className="w-full"
                          aria-label={t("payments.upsert.form.paymentMethod.label") as string}
                          dataCy="payment-method-trigger"
                        >
          ...
          <DialogFooter className="flex justify-end space-x-2">
            ...
            <Button
              ...
              disabled={hasNegativeTotalError || !selectedInvoice}
              dataCy="payment-submit"
            >
