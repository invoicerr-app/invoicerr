...
export function PaymentTable() {
  ...
  const { handleExport } = useTableExport(
    ...
    [
      {
        header: t("payments.table.columns.number"),
        cell: (payment) => payment.rawNumber || (payment.number?.toString() ?? ""),
      },
      {
        header: t("payments.table.columns.invoice"),
        cell: (payment) => payment.invoice?.rawNumber || payment.invoice?.number?.toString() || "",
      },
      ...
      {
        header: t("payments.table.columns.paidAt"),
        cell: (payment) => (payment.paidAt ? new Date(payment.paidAt).toLocaleDateString() : ""),
      },
    ...
    <Card className="gap-0">
      <CardContent className="p-4 sm:p-6 space-y-4">
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground text-center py-12">{t("payments.table.emptyState")}</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {rows.map((payment) => (
                <TableRow key={payment.id}>
                  ...
                  <TableCell>
                    {payment.invoice?.rawNumber || payment.invoice?.number || invoiceLabel(payment.invoiceId)}
                  </TableCell>
