...
import { Invoice, InvoiceItem, PaymentMethod, Receipt } from "@/types";
...
export function ReceiptUpsert({
	receipt,
	open,
	onOpenChange,
}: ReceiptUpsertDialogProps) {
	...
	const [items, setItems] = useState<Item[]>(
		receipt?.items.map((item) => ({
			...
			description:
				receipt.invoice?.items.find(
					(invItem) => invItem.id === item.invoiceItemId,
				)?.description || "",
	...
	const { data: invoices } = useGet<Invoice[]>(
		`/api/invoices/search?query=${searchTerm}`,
	);
	const { data: paymentMethods } =
		useGet<PaymentMethod[]>(`/api/payment-methods`);
	const { trigger: createTrigger, loading: createLoading } =
		usePost("/api/receipts");
			...
			<Dialog open={open} onOpenChange={handleOpenChange}>
				...
				>
					<DialogHeader className="h-fit">
						<DialogTitle>
							{t(`receipts.upsert.title.${isEdit ? "edit" : "create"}`)}
						</DialogTitle>
					</DialogHeader>
					<Form {...form}>
						<form onSubmit={form.handleSubmit(onSubmit)} data-cy="receipt-form">
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel required>
											{t("receipts.upsert.form.invoice.label")}
										</FormLabel>
										<FormControl>
											<SearchSelect
												...
												onValueChange={(val) => {
													...
													setSelectedInvoice(
														invoices?.find((inv) => inv.id === val) || null,
													);
													setSelectedItem(null);
												}}
												onSearchChange={setSearchTerm}
												placeholder={t(
													"receipts.upsert.form.invoice.placeholder",
												)}
												noResultsText={t(
													"receipts.upsert.form.invoice.noResults",
												)}
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel>
											{t("receipts.upsert.form.paymentMethod.label")}
										</FormLabel>
										<FormControl>
											<Select
												value={field.value ?? ""}
											...
											>
												<SelectTrigger
													...
													aria-label={
														t(
															"receipts.upsert.form.paymentMethod.label",
														) as string
													}
												>
													<SelectValue
														placeholder={t(
															"receipts.upsert.form.paymentMethod.placeholder",
														)}
													/>
												</SelectTrigger>
												<SelectContent>
							...
							<FormItem className="flex flex-col gap-2 mt-2">
								<FormLabel className="mb-0">
									{t("receipts.upsert.form.items.label")}
								</FormLabel>
										...
										<FormControl>
											<SearchSelect
												options={(selectedInvoice?.items || [])
													.filter(
														(item) =>
															!items.some((i) => i.invoiceItemId === item.id),
													)
													.map((item) => ({
												...
												onSearchChange={setSearchTerm}
												placeholder={t(
													"receipts.upsert.form.items.placeholder",
												)}
												noResultsText={t(
													"receipts.upsert.form.items.noResults",
												)}
											/>
										</FormControl>
										<FormMessage />
											...
											<FormItem className="flex-1">
												<FormControl>
													<BetterInput
														...
														placeholder={t(
															"receipts.upsert.form.items.description.placeholder",
														)}
														onChange={(e) =>
															onEditItem(index, "description")(e.target.value)
														}
														disabled
													/>
												</FormControl>
											...
											<FormItem>
												<FormControl>
													<BetterInput
														...
														placeholder={t(
															"receipts.upsert.form.items.amountPaid.placeholder",
														)}
														onChange={(e) =>
															onEditItem(
																index,
