...
export const ReceiptList = forwardRef<ReceiptListHandle, ReceiptListProps>(
  (
    ...
    const { trigger: triggerSendToClient } = usePost<{ message: string }>(`/api/receipts/send`);
                        ...
                        <div className="flex-1">
                          <div className="flex flex-wrap items-center gap-2">
                            <h3 className="font-medium text-foreground break-words">
                              {t('receipts.list.item.title', {
                                number: receipt.rawNumber || receipt.number,
                              })}
                            </h3>
                          </div>
                            ...
                            <div className="hidden sm:grid sm:grid-cols-1 lg:grid-cols-2 gap-1">
                              <span>
                                <span className="font-medium text-foreground">
                                  {t('receipts.list.item.invoice')}:
                                </span>{' '}
                                {receipt.invoice?.rawNumber ||
                                  receipt.invoice?.number ||
                                  t('receipts.list.item.noInvoice')}
                              </span>
                              <span>
                                <span className="font-medium text-foreground">
                                  {t('receipts.list.item.totalItemCount')}:
                                </span>{' '}
                                {receipt.items.length}
                              </span>
                              <span>
                                <span className="font-medium text-foreground">
                                  {t('receipts.list.item.totalPaid')}:
                                </span>{' '}
