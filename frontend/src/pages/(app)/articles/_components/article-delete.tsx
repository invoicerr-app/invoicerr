  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await authenticatedFetch(
        `${import.meta.env.VITE_BACKEND_URL || ""}/api/articles/${article.id}`,
        {
          method: "DELETE",
        },
      )
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>
            {article?.name
              ? `${t("articles.actions.delete") || "Delete"} ${article.name}`
              : t("articles.actions.delete") || "Delete article"}
          </DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            {t("articles.delete.description", { name: article?.name }) ||
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button variant="destructive" onClick={handleDelete} disabled={loading}>
              {loading
                ? t("articles.actions.deleting") || "Deleting..."
                : t("articles.actions.delete") || "Delete"}
            </Button>
          </div>
        </div>
      </DialogContent>
