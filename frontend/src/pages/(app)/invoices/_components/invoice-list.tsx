...
export const InvoiceList = forwardRef<InvoiceListHandle, InvoiceListProps>(
  (
    ...
    const { trigger: triggerSendInvoiceByEmail, loading: sendInvoiceByEmailLoading } =
        ...
        <Card className="gap-0">
          <CardHeader className="border-b flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:justify-between">
            ...
            <div className="flex items-center gap-2 sm:ml-auto">
              {onStatusFilterChange && (
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    ...
                    variant="outline"
                    className={`cursor-pointer text-sm px-3 py-1 rounded-full transition-all border-transparent ${
                      statusFilter?.includes("corrected")
              ...
              {showCreateButton && (
                <Button
                  onClick={() => (onAddClick ? onAddClick() : setCreateInvoiceDialog(true))}
                  dataCy="invoice-add-button"
                >
                        ...
                        <div className="flex-1">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {invoice.kind && invoice.kind !== "INVOICE" && (
                              <Badge
                                variant="secondary"
                                className={`text-xs ${getInvoiceKindColor(invoice.kind)}`}
                                data-cy="invoice-kind"
                              >
                          ...
                          <div className="mt-2 flex flex-col gap-2 text-sm text-muted-foreground">
                            <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-1">
                              ...
                              {invoice.status === InvoiceStatus.UPCOMING ? (
                                <span>
                                  <span className="font-medium text-foreground">
                                    {t("invoices.list.item.nextInvoiceDate")}:
                                  </span>{" "}
