...
export function RecurringInvoiceViewDialog({
  ...
  return (
    <Dialog open={!!recurringInvoice} onOpenChange={onOpenChange}>
      <DialogContent
        className="max-w-[95vw] lg:max-w-3xl max-h-[90dvh] flex flex-col"
        data-cy="recurring-invoice-view-dialog"
      >
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="text-xl font-semibold">{t("recurringInvoices.view.title")}</DialogTitle>
          <DialogDescription className="text-muted-foreground">
            {t("recurringInvoices.view.description")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="overflow-auto mt-2 flex-1 flex flex-col gap-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-muted/50 p-4 rounded-lg">
            <div>
              ...
              <p className="font-medium">
                {recurringInvoice.client?.name ||
                  ...
                  recurringInvoice.clientId}
              </p>
            </div>

            <div>
              <p className="text-sm text-muted-foreground">
                {t("recurringInvoices.view.fields.paymentMethod")}
              </p>
              <p className="font-medium">
            ...
            <div>
              <p className="text-sm text-muted-foreground">
                {t("recurringInvoices.view.fields.generatedCount")}
              </p>
              <p className="font-medium">{recurringInvoice._count?.generatedInvoices ?? "0"}</p>
            </div>
          </div>
          ...
          {recurringInvoice.generatedInvoices && recurringInvoice.generatedInvoices.length > 0 && (
            <div className="bg-muted/50 p-4 rounded-lg">
              <p className="text-sm text-muted-foreground mb-2">
                {t("recurringInvoices.view.fields.generatedInvoices")}
              </p>
              <div className="divide-y rounded border bg-background">
                {recurringInvoice.generatedInvoices.map((inv) => (
