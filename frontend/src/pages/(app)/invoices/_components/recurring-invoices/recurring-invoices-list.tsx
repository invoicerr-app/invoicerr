...
export const RecurringInvoiceList = forwardRef<
	RecurringInvoiceListHandle,
...
>(
	(
		...
		const [createRecurringInvoiceDialog, setCreateRecurringInvoiceDialog] =
			useState<boolean>(false);
		const [editRecurringInvoiceDialog, setEditRecurringInvoiceDialog] =
			useState<RecurringInvoice | null>(null);
		const [viewRecurringInvoiceDialog, setViewRecurringInvoiceDialog] =
			useState<RecurringInvoice | null>(null);
		const [deleteRecurringInvoiceDialog, setDeleteRecurringInvoiceDialog] =
			useState<RecurringInvoice | null>(null);
							...
							<div className="flex space-x-2">
								<Button onClick={() => setCreateRecurringInvoiceDialog(true)}>
									...
									<span className="hidden md:inline-flex">
										{t("recurringInvoices.list.actions.addNew")}
									</span>
								</Button>
							</div>
						)}
										...
										<div key={recurringInvoice.id} className="p-4 sm:p-6">
											<div className="flex flex-row sm:items-center sm:justify-between gap-4">
															...
															<div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-1">
																...
																<span>
																	...
																	{t("common.valueWithCurrency", {
																		currency: recurringInvoice.currency,
																		amount:
																			recurringInvoice.totalTTC.toFixed(2),
																	})}
																</span>
															</div>
														</div>
												...
												<div className="grid grid-cols-2 lg:flex justify-start sm:justify-end gap-1 md:gap-2">
													...
													<Button
														tooltip={t(
															"recurringInvoices.list.tooltips.delete",
														)}
														variant="ghost"
														size="icon"
