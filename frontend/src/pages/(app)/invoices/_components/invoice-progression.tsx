...
function getInvoiceActions(
  ...
  switch (currentKey) {
    case "draft":
      return handlers.onIssue ? [{ action: "issue", label: "invoices.progression.actions.issue" }] : []
    case "issued":
    case "cleared":
      return handlers.onSend ? [{ action: "send", label: sendLabel }] : []
    ...
    case "paid":
      return handlers.onArchive ? [{ action: "archive", label: "invoices.progression.actions.archive" }] : []
    default:
      return []
  }
}
    ...
    <>
            ...
            <div className="divide-y">
              {invoices.map((invoice) => {
                ...
                const isPartiallyPaid =
                  (currentStep?.key === "sent" || currentStep?.key === "delivered") && alreadyPaid > 0
                const percentPaid =
                  invoice.totalTTC > 0 ? Math.min(100, Math.round((alreadyPaid / invoice.totalTTC) * 100)) : 0
                  ...
                  >
                    <div className="min-w-0 sm:w-44 sm:flex-shrink-0">
                      ...
                      {(() => {
                        ...
                        const dotColor = ["CLEARED", "DELIVERED", "ACCEPTED", "REPORTED"].includes(cs)
                          ? "bg-emerald-500"
                    ...
                    <div className="flex-1 min-w-0 sm:max-w-md">
                      <div className="flex items-center justify-between gap-3 mb-2">
                        <span
                          className={cn("inline-flex items-center gap-2 text-sm font-semibold", colors.text)}
                        >
                        ...
                        actions.map((action, index) => (
                          <Button
                            ...
                            className={cn(
                              ...
                              index === actions.length - 1 && "bg-blue-600 text-white hover:bg-blue-700",
                            )}
                            onClick={() =>
      ...
      <AlertDialog open={!!confirmDialog} onOpenChange={() => setConfirmDialog(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {confirmDialog && t(`invoices.progression.confirmations.${confirmDialog.action}.title`)}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {confirmDialog &&
                t(`invoices.progression.confirmations.${confirmDialog.action}.description`, {
                  number: invoiceLabel,
                  client: clientName,
                })}
            </AlertDialogDescription>
