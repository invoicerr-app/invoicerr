...
export function ExpenseUpsert({ expense, open, onOpenChange }: ExpenseUpsertProps) {
  ...
  const onSubmit = async (data: ExpenseFormValues) => {
    try {
      ...
      console.error(err)
      toast.error(isEdit ? t("expenses.upsert.messages.updateError") : t("expenses.upsert.messages.addError"))
    }
  }
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      {...field}
                      placeholder={t("expenses.upsert.form.notes.placeholder")}
                      className="max-h-32"
                    />
                  </FormControl>
                  <FormMessage />
