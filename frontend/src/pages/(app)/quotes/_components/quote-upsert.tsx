...
import { Client, PaymentMethod, Quote } from "@/types";
import { PaymentMethodType, type QuoteItemType } from "@/types";
...
export function QuoteUpsert({
	quote,
	open,
	onOpenChange,
}: QuoteUpsertDialogProps) {
		...
		items: z.array(
			z.object({
				...
				quantity: z
					.number({
						invalid_type_error: t(
							"quotes.upsert.form.items.quantity.errors.required",
						),
					})
				...
				unitPrice: z
					.number({
						invalid_type_error: t(
							"quotes.upsert.form.items.unitPrice.errors.required",
						),
					})
				...
				vatRate: z
					.number({
						invalid_type_error: t(
							"quotes.upsert.form.items.vatRate.errors.required",
						),
					})
	...
	const { data: clients } = useGet<Client[]>(
		`/api/clients/search?query=${searchTerm}`,
	);
						...
						>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel required>
											{t("quotes.upsert.form.client.label")}
										</FormLabel>
										<FormControl>
											<SearchSelect
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel>
											{t("quotes.upsert.form.currency.label")}
										</FormLabel>
										<FormControl>
											<CurrencySelect
												value={field.value}
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel>
											{t("quotes.upsert.form.validUntil.label")}
										</FormLabel>
										<FormControl>
											<DatePicker
												className="w-full"
												...
												placeholder={t(
													"quotes.upsert.form.validUntil.placeholder",
												)}
											/>
										</FormControl>
										<FormMessage />
										...
										<FormControl>
											...
											>
												<SelectTrigger>
													<SelectValue
														placeholder={t(
															"quotes.upsert.form.paymentMethod.placeholder",
														)}
													/>
												</SelectTrigger>
												<SelectContent>
																...
																<FormItem>
																	<FormControl>
																		<Select
																			...
																			onValueChange={(val) =>
																				field.onChange(val as QuoteItemType)
																			}
																		>
																			<SelectTrigger
																				className="w-32 mb-0"
																...
																<FormItem>
																	<FormControl>
																		<BetterInput
																			...
																			postAdornment={t(
																				`quotes.upsert.form.items.quantity.unit`,
																			)}
																			type="number"
																			step="0.001"
																			placeholder={t(
