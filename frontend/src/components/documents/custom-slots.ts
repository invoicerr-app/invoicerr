 ...
 * type add code IN ADDITION to the generic render, never IN PLACE of it: the generic list/page
 * render exactly as they always do, unconditionally, and only ADD whatever happens to be registered
 * here for their own (typeId, slot). Nothing that READS this registry (document-list.tsx and
 * document-detail.tsx today) ever names a document type — only a REGISTRATION does, and by design
 * that can only happen in one place: custom-registrations.ts, the sole file allowed to import a
 ...
 *    descriptor itself declares (see document-list.tsx), AND in the detail page's header action
 *    cluster (document-detail.tsx) — the same component serves both, receiving the same
 *    descriptor/instance pair. Registered for "invoice" (custom/invoice-correction-routes-button.tsx)
 *    and for "received-invoice" (custom/received-invoice-download-button.tsx).
 ...
 *
 * A future slot (e.g. inside the create dialog) is added the same way: pick a new slot name,
 ...
 * used to silently OVERWRITE the first (a plain `Map.set`), which made the second "invoice" row
 * extension quietly delete the first the moment custom-registrations.ts imported both. Appending
 * instead of replacing is what makes this genuinely "additive" for more than one extension at a
 * time, the property this whole module's header already claimed.
 */
