            ...
            .union([
              ...
              z.coerce.number().refine(
                (val) => {
                  ...
                  if (numberField.pattern && !new RegExp(numberField.pattern).test(val.toString()))
                    return false
                  return true
                },
                {
...
export function DynamicFormModal({
  ...
  // Hooks must run unconditionally (same order every render) — the empty-config early return
  // therefore lives AFTER them; fields fall back to [] so the throwaway form is harmless.
  const fields = config?.form?.fields ?? []
  const schema = generateZodSchema(fields)
  const defaultValues = generateDefaultValues(fields, currentValues)
