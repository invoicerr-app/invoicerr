	...
	fields.forEach((field) => {
		...
		switch (field.type) {
						...
						.union([
							...
							z.coerce.number().refine(
								(val) => {
									...
									if (
										numberField.pattern &&
										!new RegExp(numberField.pattern).test(val.toString())
									)
										return false;
									return true;
								},
			...
			case "switch": {
				fieldSchema = z
					.boolean()
					.default((field as SwitchField).default ?? false);
				break;
			}
...
export function DynamicFormModal({
	...
	const schema = useMemo(
		() => generateZodSchema(config?.form?.fields || []),
		[config?.form?.fields],
	);
	const defaultValues = useMemo(
		() => generateDefaultValues(config?.form?.fields || [], currentValues),
		[config?.form?.fields, currentValues],
	);
	...
	const renderField = (
		...
		switch (field.type) {
			...
			case "select":
				return (
					<Select value={String(value)} onValueChange={onChange} name={name}>
						<SelectTrigger>
