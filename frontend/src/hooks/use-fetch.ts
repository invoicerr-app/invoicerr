...
export function useGetRaw<T = unknown>(
	url: string,
	options?: RequestInit,
): UseGetResult<T> {
...
type UseRequestOptions = RequestInit & { body?: unknown };

type UsePostResult<T> = {
	trigger: (body?: unknown, extraOptions?: RequestInit) => Promise<T | null>;
	data: T | null;
	loading: boolean;
	error: Error | null;
};
...
function createMethodHook(method: string) {
	return function useRequest<T = unknown>(
		url: string,
