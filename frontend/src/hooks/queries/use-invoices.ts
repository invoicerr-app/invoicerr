...
export function useInvoices(page: number) {
  return useApiQuery<InvoicesListResponse>(queryKeys.invoices.list(page), `/api/invoices?page=${page}`)
}
...
export function useInvoicesTable(filters: InvoicesTableFilters) {
  ...
  return useApiQuery<Invoice[]>(queryKeys.invoices.table(filters), `/api/invoices/table?${params.toString()}`)
}
