...
export function useQuotes(page: number) {
  return useApiQuery<QuotesListResponse>(queryKeys.quotes.list(page), `/api/quotes?page=${page}`)
}
...
export function useQuotesTable(filters: QuotesTableFilters) {
  ...
  return useApiQuery<Quote[]>(queryKeys.quotes.table(filters), `/api/quotes/table?${params.toString()}`)
}
