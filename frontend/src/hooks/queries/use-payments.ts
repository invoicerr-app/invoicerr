...
export function usePayments(page: number) {
  return useApiQuery<PaymentsListResponse>(queryKeys.payments.list(page), `/api/payments?page=${page}`)
}
...
export function usePaymentsTable(filters: PaymentsTableFilters) {
  ...
  return useApiQuery<Payment[]>(queryKeys.payments.table(filters), `/api/payments/table?${params.toString()}`)
}
